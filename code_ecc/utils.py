...
def load_checkpoint(config, model, optimizer, lr_scheduler, logger):
    ...
    if not config.EVAL_MODE and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
        
        optimizer_state = checkpoint.get('optimizer', None)
        if optimizer_state:
            match = (
                len(optimizer.param_groups) == len(optimizer_state['param_groups']) and
                all(
                    len(pg['params']) == len(state_pg['params'])
                    for pg, state_pg in zip(optimizer.param_groups, optimizer_state['param_groups'])
                )
            )
            if match:
                optimizer.load_state_dict(optimizer_state)
            else:
                logger.warning("Optimizer state in checkpoint does not match current optimizer; skipping optimizer state load.")
